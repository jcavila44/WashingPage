...
public class ColorController {
	...
	public String insertar(@Valid Color us, Model model) {
		Serv.save(us);
		return "redirect:All";
	}

	@RequestMapping(path = "/update/{id}", method = RequestMethod.POST)
	public String update(@Valid Color us, @PathVariable("id") long id, Model model) {
		us.setId_Color(id);
		Serv.update(us);
		return "redirect:../All";
	}
	...
	@GetMapping(path = "/all")
	public List<Color> listarall() {
		return Serv.listar();
	}

	@GetMapping(path = { "/{id}" })
	public Optional<Color> listarId(@PathVariable("id") long id) {
	...
	public String listarAll(Model model) {
		...
		model.addAttribute("colores", Serv.listar());

		return "/views/colors/formList";
	}
	...
	public String listar(@PathVariable("id") Long id, Map<String, Object> model, RedirectAttributes flash) {
		...
		if (id > 0) {
			colors = Serv.listarId(id);
			if (colors == null) {
				flash.addFlashAttribute("error", "El ID del Color no existe!");
				return "redirect:/views/colors/formList";
			}
		} else {
			flash.addFlashAttribute("error", "El ID del Color no puede ser cero!");
			return "redirect:/views/colors/formList";
		}
		...
		model.put("version", version);
		model.put("update", update);
		model.put("colores", colors);

		return "/views/colors/formEdit";
	}
